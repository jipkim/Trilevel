...
using JuMP, KNITRO
using Distributions
function MP(testsystem, s, kappa, PolicyB, timeset, υ0, gpmax_iter, gpo_iter, dp_iter, iter, RegGap, UserGap, bounds::bounds,
   peaktime, α0inv, π0)
   ...
   #---------------------------------------------------------------------------------#
   @variables(m, begin
      0 <= Cinv_total_c[n in busset, j in 1:iter]
      fp_c[l in lineset, t in timeset, j in 1:iter]
      0 <= gp_c[g in union(genset, genset_hat), t in timeset, j in 1:iter]
      theta_c[n in busset, t in timeset, j in 1:iter]
      xi_c[l in lineset, t in timeset, j in 1:iter]
      0 <= lambda_c[n in busset, t in timeset, j in 1:iter] <= 500
      0 <= gammaLB_c[g in union(genset, genset_hat), t in timeset, j in 1:iter] <= 1e6
      0 <= gammaUB_c[g in union(genset, genset_hat), t in timeset, j in 1:iter] <= 1e6
      0 <= deltaLB_c[l in lineset, t in timeset, j in 1:iter] <= 1e6
      0 <= deltaUB_c[l in lineset, t in timeset, j in 1:iter] <= 1e6
   end)
   #---------------------------------------------------------------------------------#
   ...
   #---------------------------------------------------------------------------------#
   @variable(m, 0.9 * Dp[n] * loadcurve[n,t] <=  Dpfun[n in busset, t in timeset] <= 1.1 * Dp[n] * loadcurve[n,t])
   @constraint(m, Dpfun_def1[n in loadzone[s], t in timeset], Dpfun[n,t] == -α0inv*(Dπ[n,t] - π0) + Dp[n] * loadcurve[n,t])
   @constraint(m, Dpfun_def2[n in setdiff(busset, loadzone[s]), t in timeset], Dpfun[n,t] == Dp[n] * loadcurve[n,t])
   ...
   @variable(m, Obj_SP_c[j in 1:iter])
   @constraint(m, Obj_SP_c_def[j in 1:iter], Obj_SP_c[j] ==
      ScaleObj*(sum(
         sum(Dπ[n,t] * dp_iter[j][n,t] for n in loadzone[s])
         - sum(lambda_c[n,t,j] * dp_iter[j][n,t] for n in loadzone[s])
         + sum(lambda_c[S_g[i],t,j] * gp_c[i,t,j] for i in S_gs[s])
         + tau_e[s] * sum(gp_c[i,t,j] for i in intersect(union(I_R, I_R_hat), S_gs[s]))
         - sum(Coperg[i][2] * gp_c[i,t,j] for i in intersect(union(I_R, I_R_hat), S_gs[s]))
         - sum(Coperg[i][2] * (gp_c[i,t,j] - α[i,t]*υ[i,t]) for i in intersect(union(I_C, I_C_hat), S_gs[s]))
      for t in timeset)
      + DCRF * tau_c[s] * sum(gpmax_iter[j][i] for i in intersect(I_R_hat, S_gs[s]))
      - sum(Cinv_total_c[n,j] for n in loadzone[s])
      )
   )
   @constraint(m, obj_cut[j in 1:iter], Obj_SP >= Obj_SP_c[j])
   ...
   #-------Cuts----------#
   @constraint(m, Cinv_def_c[n in busset, j in 1:iter], Cinv_total_c[n,j] == sum(Cinvg[g] * gpmax_iter[j][g] for g in intersect(genset_hat, B_gn[n])))
   @constraint(m, DCPF_c[l in lineset, t in timeset, j in 1:iter], fp_c[l,t,j] == (1/lines[l].x) * (theta_c[lines[l].fbus,t,j] - theta_c[lines[l].tbus,t,j]))
   @constraint(m, slack_c[t in timeset, j in 1:iter], theta_c[1,t,j] == 0)
   @constraint(m, NodeBalance_c[n in busset, t in timeset, j in 1:iter], sum(gp_c[g,t,j] for g in B_gn[n]) + sum(fp_c[l,t,j] for l in buses[n].inline)
      - sum(fp_c[l,t,j] for l in buses[n].outline) == dp_iter[j][n,t])


   @constraint(m, gpLB_c[g in union(genset, genset_hat), t in timeset, j in 1:iter], gp_c[g,t,j] >= 0)
   @constraint(m, gpUB_c[g in union(genset, genset_hat), t in timeset, j in 1:iter], gp_c[g,t,j] <= gpo_iter[j][g,t])
   @constraint(m, LineCapLB_c[l in lineset, t in timeset, j in 1:iter], fp_c[l,t,j] >= - lines[l].u)
   @constraint(m, LineCapUB_c[l in lineset, t in timeset, j in 1:iter], fp_c[l,t,j] <= + lines[l].u)

   @variable(m, Obj_prim_c[t in timeset, j in 1:iter])
   @variable(m, Obj_dual_c[t in timeset, j in 1:iter])
   @constraint(m, Oprim_c[t in timeset, j in 1:iter], Obj_prim_c[t,j] == sum(- Coperg[g][2] * gp_c[g,t,j] for g in union(genset, genset_hat)))
   @constraint(m, Odual_c[t in timeset, j in 1:iter], Obj_dual_c[t,j] ==
      sum(gpo_iter[j][g,t]*gammaUB_c[g,t,j] - 0 * gammaLB_c[g,t,j] for g in union(genset, genset_hat))
      + sum(lines[l].u * deltaUB_c[l,t,j] + lines[l].u * deltaLB_c[l,t,j] for l in lineset)
      - sum(dp_iter[j][n,t] * lambda_c[n,t,j] for n in busset)
      )
   @constraint(m, const_gp_c[g in union(genset, genset_hat), t in timeset, j in 1:iter], - gammaLB_c[g,t,j] + gammaUB_c[g,t,j] - lambda_c[B_g[g],t,j] == - Coperg[g][2])
   @constraint(m, const_fp_c[l in lineset, t in timeset, j in 1:iter], - deltaLB_c[l,t,j] + deltaUB_c[l,t,j] + xi_c[l,t,j] - lambda_c[lines[l].tbus,t,j] + lambda_c[lines[l].fbus,t,j] == 0)
   @constraint(m, const_theta_c[n in busset, t in timeset, j in 1:iter], - sum(xi_c[l,t,j]/lines[l].x for l in buses[n].outline) + sum(xi_c[l,t,j]/lines[l].x for l in buses[n].inline) == 0)
   @constraint(m, SD_split_c[t in timeset, j in 1:iter], Obj_dual_c[t,j] - Obj_prim_c[t,j] == 0)
   ##########################################################################################
   optimize!(m)
   return m
end
